...
import com.lizhaoxuan.im.protocol.packet.impl.*;
...
public class IMClient {
    ...
    private void init() {
        ...
        Class.forName(LogoutRequestPacket.class.getName());
        Class.forName(LogoutResponsePacket.class.getName());
    }
        ...
        bootstrap
                ...
                .handler(new ChannelInitializer<NioSocketChannel>() {
                    ...
                    protected void initChannel(NioSocketChannel nioSocketChannel) throws Exception {
                        ...
                        // 连接后自动登录处理器，屏蔽后，直接给服务端发送消息会因为没有登录而被关闭
    ...
    private void startConsoleThread(Channel channel) {
        ...
        new Thread(() -> {
            while (!closed){
                ...
                if (!channel.isOpen()){
                    // 连接被关闭
                    log.warn("[IMClient] connect is closed!");
                    closed = true;
                }
                // 登录检查，不检查登录标识，直接发请求给服务端，验证服务端登录状态校验
                if (SessionManager.isLogin(channel)){
                    ...
                    if (line.equals("exit")){
                        // 登出请求
                        LogoutRequestPacket logoutRequestPacket = LogoutRequestPacket.builder().build();
                        byteBuf = PacketCodeC.encode(logoutRequestPacket);
                    }else {
                        // 构建消息数据包
                        MessageRequestPacket messageRequestPacket = MessageRequestPacket.builder()
                                .toUserId(line.split(":")[0].trim())
                                .message(line.split(":")[1].trim()).build();
                        byteBuf = PacketCodeC.encode(messageRequestPacket);
                    }
                    // 发送给服务端
